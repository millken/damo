...
<script lang="ts">
    let { id, ref, class: className, style, layout, children }: LayoutProps = $props();
...
<div bind:this={el} aria-label="Layout" {id} class={layoutVariants({ layout, grow: isGrow, className })} {style}>
    {@render children?.()}
</div>
