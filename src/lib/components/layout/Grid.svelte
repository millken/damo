<script lang="ts">
    ...
    const {
        ...
        style,
        ...rest
    } = $derived(extractProps(props, gridPropDefs, layoutPropDefs, marginPropDefs));

</script>

<svelte:element this="{as}" class:rt-Grid={true} class={className} {...rest} style={styleToString(style)}>
    {@render children?.()}
</svelte:element>